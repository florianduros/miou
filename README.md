...
The bot polls data from the Terraforming Mars server at a configured interval (specified in your [`config.toml` as `polling_interval`](#polling-interval)). The alert triggers after the bot detects that it's the player's turn (depending on the polling interval) and the specified delay has passed.

## Installation

### Docker

Docker is the recommended way to run Miou. You can use the following `docker-compose.yml` file:
```yaml
services:
  miou:
    ...
    volumes:
      ...
      - ./miou.toml:/config/miou.toml
      # Mount your data directory (required)
      - ./data:/data
    environment:
      # Set log level, info by default (optional)
      - RUST_LOG=debug
```
...
Run
```bash
miou --config config.toml --data-path ./data
```
...
3. Create a `config.toml` file (see Configuration section below)

4. Run the bot:
```bash
cargo run --release -- --config config.toml --data-path ./data
```
